...
function caveman(text, level = 'full', lang = 'mixed') {
    ...
    // ============================================
    // [한국어 압축 규칙] — level 별 분기
    // Lite: 인사 + 약한 한정사만 (문법 유지)
    // Full: + 존댓말, 주어, 접속사, 약한표현, 이유, 마무리, 군더더기 서두
    // Ultra: + 추가 공격적 압축 (시간/상태 표현, 보조용언)
    // ============================================
    if (lang === 'ko' || lang === 'mixed') {
        ...
        // [Ultra] 추가 공격적 압축 — 보조용언, 시간 부사, 상태/생각 표현
        if (level === 'ultra') {
            result = result
                .replace(/\s*(지금|당장|이제|곧|이미)\s*/g, ' ')
                .replace(/\s*(생각하고|생각하며|생각합니다|생각이|생각한다|생각해)\s*/g, ' ')
                .replace(/\s*(있고|있는|있으며|있습)\s*/g, ' ')
                .replace(/\s*(상태이기?|상태로?|상태에?)\s*/g, ' ')
                .replace(/\s*(무언가를?|뭔가를?)\s*/g, ' ');
        }
    }
